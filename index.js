...
function calculateExpression(expression) {
  if (typeof expression === "string") {//Преревіряємо чи аргумент є рядком
...
function getNumberFromString(s) {
  const number = parseFloat(s); // Використовуємо функцію parseFloat для того, щоб спробувати отримати число з рядка
  if (isNaN(number)) { // Перевіряємо, чи є отримане значення числом за допомогою isNaN
  return "Couldn't extract a number from the provided string."; // Якщо значення не є числом, повертаємо текст "Couldn't extract a number from the provided string."
    ...
    return number; // Якщо значення є числом, повертаємо його
  }
}
...
function getIntegerFromString(s) {
  const number = parseInt(s); // Використовуємо функцію parseInt для того, щоб спробувати отримати ціле число з рядка
  if (isNaN(number)) { // Перевіряємо, чи є отримане значення числом за допомогою isNaN
  return "Couldn't extract a number from the provided string."; // Якщо значення не є числом, повертаємо текст "Couldn't extract an integer from the provided string."
} else { 
  return number; // Якщо значення є числом, повертаємо його
}
}
...
function isNumberFinite(num) {
  if (isFinite(num)) { // Використовуємо вбудовану функцію isFinite, щоб перевірити, чи є введене число скінченним.
...
function isValueNaN(value) {
  if (isNaN(value)) { // Використовуємо вбудовану функцію isNaN, щоб перевірити, чи є введене значення NaN.
...
function createURLObject(urlStr) {
  const url = new URL(urlStr); //Використовуємо вбудований конструктор URL, щоб створити новий об`єкт URL з введеного рядка.
  return url; // Повертаємо створений об'єкт URL.
}
...
function encodeURLComponent(urlComponent) {
  const encodeComponent = encodeURIComponent(urlComponent); // Використовуємо вбудовану функцію encodeURIComponent, щоб закодувати введений компонент URL.
  // Ця функція повертає закодований компонент URL, замінивши небезпечні символи на їх процентне кодування.
  return encodeComponent;// Повертаємо закодований компонент URL.
}
...
function encodeURL(url) {
  const encodeURL = encodeURI(url); // Використовуємо вбудовану функцію encodeURI, щоб закодувати введений URL.
  // Ця функція повертає закодований URL, замінивши небезпечні символи на їх процентне кодування.
  return encodeURL; // Повертаємо закодований URL.
}
...
function decodeURLComponent(urlComponent) {
  const decodeURLComponent = decodeURIComponent(urlComponent); // Використовуємо вбудовану функцію decodeURIComponent, щоб декодувати введений закодований компонент URL.
  // Ця функція повертає декодований компонент URL, замінивши процентне кодування символів на їх реальні значення.
  return decodeURLComponent// Повертаємо декодований компонент URL.
}
...
function decodeURL(url) {
  const decodeURL = decodeURI(url);  // Використовуємо вбудовану функцію decodeURI, щоб декодувати введений закодований URL.
  // Ця функція повертає декодований URL, замінивши процентне кодування символів на їх реальні значення.
  return decodeURL; // Повертаємо декодований URL.
}
